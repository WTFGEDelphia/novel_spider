...
def main():
    ...
    if system == "windows":
        ...
        # 构建 Windows 命令
        shell_flag = True
        # command = f'cmd /c "{venv_activate} && cd /d {spider_dir} && scrapy crawl auto_novel_top100"'
        command = f'cmd /c "{venv_activate} && cd /d {spider_dir} && scrapy crawl auto_novel_top100 -a local=1"'
        ...
        if not os.path.exists(venv_activate):
            ...
            sys.exit(1)
        # 构建 shell 命令
        # command = f"source {venv_activate} && cd {spider_dir} && scrapy crawl auto_novel_top100"
        command = f"source {venv_activate} && cd {spider_dir} && scrapy crawl auto_novel_top100 -a local=1"
        shell_flag = True
