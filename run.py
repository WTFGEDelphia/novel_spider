...
class VenvManager:
    ...
    def check_venv(self, check_python: bool = True) -> bool:
        ...
        if not venv_activate.exists():
            print_log("ERROR", f"虚拟环境未找到: {venv_activate}")
            return False

        if check_python and not python_exe.exists():
            print_log("ERROR", f"Python可执行文件未找到: {python_exe}")
            return False

        return True

    def build_command(
        self, command: str, check_python: bool = True
        ...
        if self.is_windows:
            full_command = (
                f'cmd /c "{venv_activate} && cd /d {self.base_dir} && {command}"'
            )
            shell_flag = True
            executable = None
        else:
...
class SpiderRunner:
    ...
    def run_spider(self, spider_name: str, local: bool = False) -> bool:
        """运行爬虫"""
        print_log("INFO", f"开始运行爬虫: {spider_name}")
        ...
        if spider_name not in valid_spiders:
            print_log("ERROR", f"无效的爬虫名称: {spider_name}")
            print_log("INFO", f"可用的爬虫: {', '.join(valid_spiders)}")
            return False
        ...
        full_command, shell_flag, executable = self.venv_manager.build_command(
            command, check_python=False
        )
        if not full_command:
            return False

        return CommandExecutor._execute_command(
            full_command, shell_flag, executable, "爬虫"
        )
...
class EbookExporter:
    ...
    def export_ebooks(self, format_type: str) -> bool:
        """导出电子书"""
        print_log("INFO", f"开始导出电子书，格式: {format_type}")

        # 验证格式
        valid_formats = ["epub", "txt"]
        if format_type not in valid_formats:
            print_log("ERROR", f"无效的导出格式: {format_type}")
            print_log("INFO", f"支持的格式: {', '.join(valid_formats)}")
            return False

        # 检查导出脚本是否存在
        export_script = self.venv_manager.base_dir / "run_export_to_ebooks.py"
        if not export_script.exists():
            print_log("ERROR", f"导出脚本未找到: {export_script}")
            return False
        ...
        return CommandExecutor._execute_command(
            full_command, shell_flag, executable, "导出"
        )
...
class CommandExecutor:
    ...
    def _execute_command(
        command: str, shell: bool, executable: Optional[str], operation_name: str
    ) -> bool:
        """执行命令"""
        print_log("INFO", f"执行命令: {command}")
        ...
        try:
            ...
            if result.returncode == 0:
                print_log("INFO", f"{operation_name}完成。")
                return True
            else:
                print_log("ERROR", f"{operation_name}失败，返回码: {result.returncode}")
                return False

        except Exception as e:
            print_log("ERROR", f"执行{operation_name}时发生错误: {e}")
            return False
