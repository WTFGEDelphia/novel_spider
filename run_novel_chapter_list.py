...
def main():
    ...
    if system == "windows":
        ...
        if not os.path.exists(scrapy_exe):
            ...
            sys.exit(1)
        # 构建 Windows 命令
        command = f'cmd /c "{venv_activate} && cd /d {spider_dir} && scrapy crawl novel_chapter_list"'
        shell_flag = True
        ...
        if not os.path.exists(venv_activate):
            ...
            sys.exit(1)
        # 构建 shell 命令
        command = f"source {venv_activate} && cd {spider_dir} && scrapy crawl novel_chapter_list"
        shell_flag = True
